...
- Python tooling lives in `pyproject.toml` + `.pre-commit-config.yaml`; run `pre-commit run --all-files` when touching `.py`.
- Core logic is decomposed into `src/core/*` and tokens in `src/design/tokens.ts`, all re-exported from `src/index.ts` for newcomers.

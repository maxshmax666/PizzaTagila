...
- `/menu` — мобильный главный экран «Меню» в телефонной рамке (карточки пицц, чипы категорий).
- `/cart` — корзина с фотографиями, степпером количества и итоговой суммой.
- `/checkout` — оформление заказа: контакты, адрес, комментарий, оплата, итоговая таблица.
- `/promos` — акции с крупными промо-блоками.
- `/delivery` — статус доставки и слот времени с картой-заглушкой.
- `/account` — контакты и быстрые действия аккаунта.
- `/notifications` — экран уведомлений/оферты.
- `/loyalty` — программа лояльности с карточками и картой.
